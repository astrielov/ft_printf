...
/*   Updated: 2018/04/18 14:02:57 by astrelov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	pre_padding_ptr(t_pf *arg, t_buff *arg_buff, size_t nbr_len)
{
	...
	if (width > precis + 2 && width > nbr_len + 2 && !(arg->flags & FLAG_MINUS) && !(arg->flags & FLAG_ZERO))
	{
		if (precis > nbr_len)
			push_chars(arg_buff, ' ', width - precis - 2);
		else
			push_chars(arg_buff, ' ', width - nbr_len - 2);
	}
}
...
void	post_padding_ptr(t_pf *arg, t_buff *arg_buff, size_t nbr_len)
{
	...
	{
		if (precis > nbr_len)
			push_chars(arg_buff, char_to_fill, width - precis - 2);
		else
			push_chars(arg_buff, char_to_fill, width - nbr_len - 2);
	}
}
